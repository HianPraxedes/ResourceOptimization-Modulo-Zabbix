...
<body>
    ...
    function updateCards() {
        ...
        // Average Efficiency: calcular a média de current_usage (removendo a "%" e ignorando "N/A") para todos os hosts
        var cpuSum = 0,
            cpuCount = 0;
        var memSum = 0,
            memCount = 0;
        hosts.forEach(function(item) {
            if (item.current_usage && item.current_usage !== 'N/A') {
                ...
                if (item.resource.toLowerCase() === 'cpu') {
                    cpuSum += usage;
                    cpuCount++;
                } else if (item.resource.toLowerCase() === 'memory' || item.resource.toLowerCase() ===
                    'memória') {
                    memSum += usage;
                    memCount++;
                }
        ...
        var avgCpu = cpuCount > 0 ? (cpuSum / cpuCount).toFixed(2) : 'N/A';
        var avgMem = memCount > 0 ? (memSum / memCount).toFixed(2) : 'N/A';
        document.getElementById('avg-efficiency-cpu').textContent = "CPU: " + (avgCpu !== 'N/A' ? avgCpu + '%' : 'N/A');
        document.getElementById('avg-efficiency-memory').textContent = "Memory: " + (avgMem !== 'N/A' ? avgMem + '%' :
            'N/A');
        ...
        document.getElementById('optimization-potential-cpu').textContent = "CPU: " + (cpuArrow ? cpuArrow + " " + Math
            .abs(cpuPotential).toFixed(2) + " vCPUs" : "No change");
        document.getElementById('optimization-potential-memory').textContent = "Memory: " + (memArrow ? memArrow + " " +
            Math.abs(memPotential).toFixed(2) + " GB" : "No change");
    }
